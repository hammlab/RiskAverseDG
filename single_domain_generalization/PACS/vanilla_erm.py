...
CHECKPOINT_PATH = "./checkpoints/vanilla_dg_resnet50_S_DG_" + METHOD + "_source_" + str(SRCS[0]) + "_trial_" + str(TRIAL)
...
optimizer_base_model = tf.keras.optimizers.SGD(learning_rate=1E-1)
optimizer_logits = tf.keras.optimizers.SGD(learning_rate=1E-1)
...
def train_step_min_erm(data, class_labels):
    ...
    with tf.GradientTape(persistent=True) as tape:
        rep = base_model(data, training=True)
