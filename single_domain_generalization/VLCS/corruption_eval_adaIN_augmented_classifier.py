...
def _sample_styles(content_image, num):
    ...
    for _ in range(ceil(num / BATCH_SIZE)):
        ...
        content_encoded_batch = encoder_vgg19(tf.cast(repeated_content_image, dtype=tf.float32))
...
for i in range(len(target_data)):
    ...
    selected_indices = mini_batch_class_balanced(y_test_list, 10)
...
for severity in [3,5]:
    ...
    for corruption in imagenet_c_corruptions.CORRUPTIONS:
        ...
        for i in range(len(trg_X_selected)):
            new_img = corruption_transform(np.uint8(trg_X_selected[i]*255)).permute(1,2,0).cpu().numpy()/255.
    ...
    print(args, severity, "Smoothed_Conf")
    print("".join(str(conf_values)))
