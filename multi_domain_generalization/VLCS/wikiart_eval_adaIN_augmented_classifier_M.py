...
from utils import load_VLCS_sources, load_VLCS_targets, eval_accuracy, mini_batch_class_balanced
...
src_data_loaders, _, _ = load_VLCS_sources(SRCS, int(min(N, BATCH_SIZE)))
...
for t_i in range(len(trg_X_wikiart)):
    
    x_content = np.array(trg_X_wikiart[t_i]).reshape([1, HEIGHT, WIDTH, NCH])
    ...
    if t_i%500 == 0 or t_i == len(trg_X_wikiart) - 1:
        print("Smoothed")
        print(t_i+1)
